...
const webpackDev = { // 开发配置文件
  ...
  devServer: { // 启动devServer，不会在本地生成文件，所有文件会编译在内存中(读取速度快)
    ...
    // publicPath: '', // 静态资源存放位置，根目录的assets文件夹，确保publicPath总是以斜杠(/)开头和结尾。可以设置为CDN地址。这个选项类似url-prefix
    port: config.devPort,
    host: '0.0.0.0', // 设置为0.0.0.0并配合useLocalIp可以局域网访问
    useLocalIp: true, // 使用本机IP打开devServer，而不是localhost
    after: function(app) {
      opn('http://localhost:' + config.devPort + config.devIndex, { app: ['chrome', '--remote-debugging-port=9222'] })
    },
