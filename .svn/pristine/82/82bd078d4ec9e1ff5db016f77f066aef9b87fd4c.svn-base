...
const hash = config.hash ? '.[contenthash:8]' : ''
const cssExtracter = new ExtractTextWebpackPlugin({
  filename: 'static/css/[name]-css' + hash + '.css', // 直接导入的css文件，提取时添加-css标识
  allChunks: true // 从所有的chunk中提取，当有CommonsChunkPlugin时，必须为true
})
const sassExtracter = new ExtractTextWebpackPlugin({
  filename: 'static/css/[name]-sass' + hash + '.css', // 直接导入的sass文件，提取时添加-sass标识
  allChunks: true
})

const webpackProd = { // 生产配置文件
  output: {
    filename: hash ? 'static/js/[name].[chunkhash:8].bundle.js' : 'static/js/[name].bundle.js', // 生产环境用chunkhash
    ...
    },
    {
      ...
      use: [{ // 图片文件小于8k时编译成dataUrl直接嵌入页面，超过8k回退使用file-loader
        ...
        options: {
          limit: 8192, // 8k
          publicPath: config.publicPath,
          name: 'static/img/[name].[ext]' // 回退使用file-loader时的名称
  ...
  plugins: [
    ...
    new CleanWebpackPlugin([config.BUILD_NAME], {
      root: config.PROJECT_PATH // 默认为__dirname，所以需要调整
    })
  ]
}
// 压缩JS
if (config.compressJs) webpackProd.plugins.push(new UglifyJSPlugin())
